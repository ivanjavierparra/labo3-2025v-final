...
def optimizar_con_optuna_xgboost_db(train, semillas=[42, 101, 202, 303, 404], version="v1", n_trials=100):
    """
    ...
    def objective(trial):
        ...
        for train_idx, val_idx in tscv.split(X):
            ...
            for seed in semillas:
                ...
                tfe = total_forecast_error(y_val_fold, preds)
                tfe_seeds.append(tfe)

            tfe_folds.append(np.mean(tfe_seeds))

        return np.mean(tfe_folds)

    # ==================== Callback ====================
    def print_best_trial(study, trial):
        print(f"Mejor trial hasta ahora: Error={study.best_value:.6f}, Parámetros={study.best_trial.params}")
    ...
    study.optimize(objective, n_trials=n_trials, callbacks=[print_best_trial], timeout=3 * 3600)
...
#     # Sample weight: log1p para suavizar
#     weights = np.log1p(y).replace([np.inf, -np.inf], 0).fillna(0).clip(lower=1e-3)
...
#                 preds = model.predict(X_val_fold)
#                 rmse = root_mean_squared_error(y_val_fold, preds, sample_weight=w_val)
#                 rmse_seeds.append(rmse)
...
#             direction='minimize',
#             # pruner=MedianPruner(n_startup_trials=10, n_warmup_steps=5),
#             ksampler=optuna.samplers.TPESampler(seed=42)
#         )
#         print("Advertencia: No se pudo conectar a SQLite. Usando almacenamiento en memoria")
